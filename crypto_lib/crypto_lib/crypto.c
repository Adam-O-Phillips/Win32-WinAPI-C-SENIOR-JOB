...
#if !defined SWAPLONG
#define SWAPLONG(x)		((ULONG)((ULONG)(x) & 0xFF000000) >> 24) | \
						((ULONG)((ULONG)(x) & 0x00FF0000) >> 8) | \
						((ULONG)((ULONG)(x) & 0x0000FF00) << 8) | \
						((ULONG)((ULONG)(x) & 0x000000FF) << 24)
...
int rsa2048_encrypt(const void* inbuf, uint32_t buflen, RSA2048_KEY_BLOB* priv_key, void** outbuf)
{
	...
	*(uint32_t*)(pkey.public_exponet + RSA_MAX_MODULUS_LEN - 4) = SWAPLONG(rsapubkey->pubexp);
...
int rsa2048_decrypt(const void* inbuf, uint32_t buflen, RSA2048_KEY_BLOB* pub_key, void** outbuf)
{
	...
	*outbuf = malloc(buflen);
	...
	klen = rsapubkey->bitlen / 8;
	pkey.bits = rsapubkey->bitlen;
	*(uint32_t*)(pkey.exponent + RSA_MAX_MODULUS_LEN - 4) = SWAPLONG(rsapubkey->pubexp);
	/*memcpy(pkey.modulus, ptr, klen);*/
	for (i = 0; i < klen; i++)
		pkey.modulus[RSA_MAX_MODULUS_LEN - i - 1] = ptr[i];
...
{
	return crc_16((const unsigned char*)input_str, num_bytes);
}
